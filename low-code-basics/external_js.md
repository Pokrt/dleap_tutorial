# Custom JavaScript™ in HCL Domino Leap
...
## Understanding JavaScript™ Objects in Domino Leap

HCL Domino Leap provides specific JavaScript™ objects that you can use within your custom scripts:

- **`app` (Application object)**: This object contains functions for accessing global information about your
  application. For example, you can use `app.getCurrentUser();` to fetch the current user's details.

  ::: warning
  todo přidat link na app object
  :::
...
- **Security Disabled**: Without JavaScript™ security, external files load through a `<script>` tag, and you must
  use `window.NitroApplication` for accessing Domino Leap API functions. Uploaded files use `eval()` for evaluation.
